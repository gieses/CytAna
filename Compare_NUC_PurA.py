...
if execute:
    for peptides, proteins in zip(peptide_files, protein_files):
        ...
        get_kinomexplorer_output(peptides_df[peptides_df["isPhospho"]==True], 
                                 FASTA_dic, 
...
for peptides, proteins, kinomes in zip(peptide_files, protein_files, kinome_files):
    kinome_df = read_kinome_explorer(kinomes)
    ...
    computelog(phospho_peptides)
    computelog(regular_peptides)
    computelog(proteins_df)
    ...
    writer = pd.ExcelWriter(peptides[:-5] + '____overview.xlsx',
                            engine='xlsxwriter')
    ...
    print "Results written to: {}".format(peptides[:-5] + '____overview.xlsx')
